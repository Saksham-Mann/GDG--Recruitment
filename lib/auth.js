...
export const auth = betterAuth({
  ...
  session: {
    ...
    cookieCache: {
      enabled: true,
      maxAge: 60 * 5, // 5 minutes (allows prompt session revocation while caching rapid requests)
    },
  ...
  emailVerification: {
    ...
    sendVerificationEmail: async ({ user }) => {
      try {
        const { createAndSendOtp } = await import("@/lib/email-otp");
        await createAndSendOtp(user.email);
      } catch (err) {
        console.error("Failed to send verification OTP on signup:", err);
      }
    },
  },
  socialProviders: {
